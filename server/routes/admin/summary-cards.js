...
router.get('/', async (req, res) => {
  try {
    ...
    const [
      ...
      query(`SELECT COUNT(*) AS total_sessions FROM scheduled_sessions WHERE 1 ${dateCondition}`),
      query(`SELECT COUNT(*) AS sessions_filtered FROM scheduled_sessions WHERE 1 ${dateCondition}`),
      query(`SELECT COUNT(*) AS completed_filtered FROM scheduled_sessions WHERE status = 'completed' ${dateCondition}`),
      query(`SELECT COUNT(*) AS cancelled_filtered FROM scheduled_sessions WHERE status = 'cancelled' ${dateCondition}`),
      query(`SELECT ROUND(AVG(stars),2) AS avg_tutor_rating FROM tutor_ratings WHERE stars IS NOT NULL`),
      query(`SELECT ROUND(AVG(stars),2) AS avg_tutee_rating FROM tutee_ratings WHERE stars IS NOT NULL`),
